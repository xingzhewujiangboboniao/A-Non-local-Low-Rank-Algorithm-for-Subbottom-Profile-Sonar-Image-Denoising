...
X = imread('testlowrank.bmp');
X = double(X);%(850:950,1:100)
...
% nFrames     = size(X,2);
% lambda      = 2e-2;
% L0          = repmat( median(X,2), 1, nFrames );%initial filtered value
% is used as L0
% S0          = X - L0;
% epsilon     = 5e-1*norm(X,'fro'); % tolerance for fidelity to data
